...
data: {
    ...
    // ✅ Profile with default avatar & subtext
    profile: { 
        name: 'User', 
        avatar: 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png',
        gender: '', 
        age: null,
        subtext: 'Ready to achieve your goals today? 🎯'
    },
    quotes: [
...
pomo: {
    ...
    loadState() {
        ...
        if (saved) {
            ...
            const settings = APP.data.pomoSettings || { work: 25 };
    ...
    updateDisplay() {
        ...
        if (statusEl) {
            statusEl.textContent = this.isRunning
                ? (this.mode === 'work' ? 'Focus Time' : this.mode === 'shortBreak' ? 'Short Break' : 'Long Break')
                : 'Ready';
        }
    },
    start() {
    ...
    exportPDF() {
        if (!window.jspdf) return APP.showToast('PDF library not loaded. Check HTML CDN links.');
...
// ✅ Update greeting UI with name, avatar & subtext
updateGreetingUI() {
    const profile = this.data.profile || { name: 'User', avatar: 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png', subtext: 'Ready to achieve your goals today? 🎯' };
    ...
    const subtext = profile.subtext || 'Ready to achieve your goals today? 🎯';
    
    // Dashboard greeting elements
    ...
    if (greetingFull) {
        ...
        greetingFull.textContent = `${greeting}, ${displayName}`;
    }
...
initProfile() {
    ...
    if (subtextInput) {
        ...
        subtextInput.addEventListener('input', (e) => {
            ...
            if (previewSub) previewSub.textContent = e.target.value || 'Ready to achieve your goals today? 🎯';
        });
    }
    
    // Avatar selection
    document.querySelectorAll('.avatar-option').forEach(option => {
        if (profile.avatar && option.dataset.avatar === profile.avatar) {
    ...
    if (nameInput) {
        nameInput.addEventListener('input', (e) => {
            const previewName = document.getElementById('preview-greeting');
            const sidebarName = document.getElementById('sidebar-username');
            const val = e.target.value.trim() || 'User';
            if (previewName) {
                const greeting = this.getGreeting();
                const displayName = val.charAt(0).toUpperCase() + val.slice(1);
                previewName.textContent = `${greeting}, ${displayName}`;
            }
    ...
    document.getElementById('profile-form')?.addEventListener('submit', (e) => {
        ...
        this.data.profile = {
            ...
            avatar: selectedAvatar ? selectedAvatar.dataset.avatar : (profile.avatar || 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png'),
            gender: document.getElementById('profile-gender').value,
            age: parseInt(document.getElementById('profile-age').value) || null,
            subtext: subtextVal ? subtextVal.slice(0, 30) : 'Ready to achieve your goals today? 🎯'
        };
...
init() {
    ...
    this.updateGreetingUI(); // ✅ Update on every page load
    ...
    if (path.includes('profile.html')) this.initProfile(); // ✅ Profile handler
},
...
load() {
    ...
    if (saved) {
        ...
        // ✅ Ensure profile with defaults for backward compatibility
        if (!parsed.profile) {
            parsed.profile = { 
                name: 'User', 
                avatar: 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png',
            ...
            // ✅ Add default subtext if missing
            if (!parsed.profile.subtext) {
                parsed.profile.subtext = 'Ready to achieve your goals today? 🎯';
            }
            // ✅ Add default avatar if missing
            if (!parsed.profile.avatar) {
                parsed.profile.avatar = 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png';
            }
        }
...
fixThemeUI() {
    ...
    if (this.data.currentTheme && this.data.currentTheme !== 'purple') {
        document.body.classList.add(`theme-${this.data.currentTheme}`);
    }
...
updateThemeIcon() {
    document.querySelectorAll('.theme-toggle i').forEach(i => i.className = this.data.darkMode ? 'fa fa-moon' : 'fa fa-sun');
},
...
showToast(msg) {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('Learny', { body: msg });
    }
},
...
shareCountdown(id) {
    ...
    if (navigator.share) {
        navigator.share({ title: 'Learny Countdown', text }).catch(() => this.copyToClipboard(text));
    } else {
